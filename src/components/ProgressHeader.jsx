...
export function ProgressHeader({
  ...
  const currentStep = Math.max(1, activeStep - 1);
  const initiativeTotal = TOTAL_STEPS - 1;
  const percentage = Math.round((currentStep / initiativeTotal) * 100);
  ...
  return (
    <header className="no-print fixed top-0 left-0 w-full h-16 bg-white border-b border-line z-50 flex items-center gap-4 px-4 md:px-8">
      ...
      <span className="text-sm font-extrabold w-16 text-right shrink-0">
        {currentStep}/{initiativeTotal}
      </span>

      <button type="button" onClick={onExit} className={`${BUTTON.quiet} shrink-0`}>
        Iniciativas
      </button>
